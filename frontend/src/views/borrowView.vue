<template>
      ...
      <template v-slot:bottom>
        <div class="text-center pa-4">
          ...
          <div class="text-caption text-grey-darken-1 mt-2">
            Total: {{ totalElements }} records | Page {{ currentPage }} of {{ totalPages }}
          </div>
        </div>
      </template>
    </v-data-table>
    ...
    <v-card>
      ...
      <v-card-text class="pa-6" v-if="selectedRecord">
        <v-row>
          ...
          <v-col cols="12">
            <div class="mb-4">
              <div class="text-caption text-grey-darken-1">Book Title</div>
          ...
          <v-col cols="12" md="4">
            <div class="mb-4">
              <div class="text-caption text-grey-darken-1">Borrow Date</div>
          ...
          <v-col cols="12" md="4">
            <div class="mb-4">
              ...
              <div class="text-body-1" :class="{ 'text-error font-weight-bold': selectedRecord.isOverdue }">
                ...
                <v-chip v-if="selectedRecord.isOverdue" color="error" size="small" class="ml-1">Overdue</v-chip>
              </div>
            </div>
          </v-col>
          <v-col cols="12" md="4">
            <div class="mb-4">
              <div class="text-caption text-grey-darken-1">Return Date</div>
              <div class="text-body-1">{{ selectedRecord.returnDate ? formatDate(selectedRecord.returnDate) : 'Not returned' }}</div>
            </div>
          </v-col>
          <v-col cols="12" md="6">
            <div class="mb-4">
  ...
  <v-dialog v-model="returnDialog" max-width="400px">
    <v-card>
      <v-card-title class="text-h5">Confirm Return</v-card-title>
      <v-card-text>
      ...
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn color="grey" variant="text" @click="cancelReturn" :disabled="returningRecordId !== null">Cancel</v-btn>
        <v-btn
          color="success"
          variant="elevated"
          @click="confirmReturn"
